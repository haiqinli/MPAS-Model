...
module dep_dry_emerson_mod
  ...
  use mpas_smoke_config, only : num_chem, p_smoke, n_dbg_lines
...
contains
    subroutine dry_dep_driver_emerson(rmol,ustar,znt,ndvel,ddvel,vgrav,   &
               ...
               drydep_flux,settling_flux,dbg_opt,                         &
...
!----------------------------------------------------------------------
 ...
 !
 ! Output arrays
       REAL(RKIND), DIMENSION( ims:ime, jms:jme, ndvel ), INTENT(INOUT) :: ddvel
       REAL(RKIND), DIMENSION( ims:ime, kms:kme, jms:jme, ndvel), &
                                                              INTENT(OUT)   :: vgrav
       REAL(RKIND), DIMENSION( ims:ime, jms:jme, ndvel ), INTENT(OUT)   :: settling_flux, drydep_flux
 ! Local
       REAL(RKIND), DIMENSION( ims:ime , jms:jme )  :: aer_res
       REAL(RKIND), DIMENSION( ndvel ) :: cblk
 ! Modpar variables, mass, density, diameter, knudsen number, mean free path
       ...
       real(RKIND), dimension(ndvel)    :: dt_settl, chem_before, chem_after
       real(RKIND), dimension( kts:kte, ndvel ) :: cblk_col, vg_col
...
subroutine particle_settling(cblk,rho_phy,delz,vg,dt_settl,ndt_settl,ndvel,kts,kte)
     ...
     REAL(RKIND), DIMENSION(kts:kte,ndvel), INTENT(IN) :: vg
     REAL(RKIND), DIMENSION(kts:kte,ndvel), INTENT(INOUT) :: cblk
     REAL(RKIND), DIMENSION(ndvel), INTENT(IN) :: dt_settl
