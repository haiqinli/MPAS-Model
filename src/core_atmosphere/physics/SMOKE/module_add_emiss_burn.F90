...
CONTAINS
  subroutine add_emis_burn(dtstep,dz8w,rho_phy,pi,ebb_min,          &
   ...
   real(RKIND), DIMENSION(ims:ime,jms:jme), INTENT(IN)     :: hwp, peak_hr, fire_end_hr, ebu_in !RAR: Shall we make fire_end integer?
      ...
      do i=its,ite
       do k=kts,kfire_max
          if (ebu(i,k,j)<ebb_min) cycle
           ...
           chem(i,k,j,p_smoke) = MIN(MAX(chem(i,k,j,p_smoke),0._RKIND),5.e+3_RKIND)
